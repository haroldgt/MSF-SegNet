...
class SemKITTI_sk(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # rgb features compution
        # bgr_feat = bgr_features(annotated_data[:26240, :], self.color_map)
        # data_tuple += (bgr_feat,)
        # var_save_txt("data_tuple", data_tuple)
        return data_tuple

class nuScene_sk(data.Dataset):
    def __init__(self, data_path, imageset='train',
                 return_ref=False, label_mapping="nuscenes.yaml",labelData_bits=8, nusc=None):
        self.return_ref = return_ref
        self.labelData_bits = labelData_bits
    ...
    def __getitem__(self, index):
        ...
        if self.labelData_bits == 8:
            annotated_data = np.fromfile(lidarseg_labels_filename, dtype=np.uint8).reshape([-1, 1])
            annotated_data = np.vectorize(self.learning_map.__getitem__)(annotated_data)
        elif self.labelData_bits == 16:
            raise ValueError("bits number of label must be 8 or other!")
        else :
            raise ValueError("bits number of label must be 8 or other!")
        raw_data = np.fromfile(os.path.join(self.data_path, lidar_path), dtype=np.float32, count=-1).reshape([-1, 5])
        ...
        # data_tuple = (raw_data[:16240, :3], annotated_data[:16240, :].astype(np.uint8))
        data_tuple = (raw_data[:, :3], annotated_data.astype(np.uint8))
        
        if self.return_ref:
            # data_tuple += (raw_data[:16240, 3],)
            data_tuple += (raw_data[:, 3],)
        # var_save_txt("data_tuple", data_tuple)
        # rgb features compution
        # bgr_feat = bgr_features(annotated_data[:26240, :], self.color_map)
...
@nb.jit('u1[:,:,:](u1[:,:,:],i8[:,:])', nopython=True, cache=True, parallel=False) # numba library accelerate
